...
public class MissingFactTest {
    @Test
    public void missingFactTest() {
        Model model;
        LogicTxtParser parser = new LogicTxtParser();
        try {
            BufferedReader file = new BufferedReader(new FileReader("missingfact.txt"));
            String line;
        ...
        if ((model = parser.getResults()) != null) {
            ...
            assertEquals(returnedResult.toString(), "[B]");
